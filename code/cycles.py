# The 5 pipeline stages of every instruction

import vars, dependencies, method
...
def Clock_Cycles(x, y, i):
    ...
    global var_counter1, var_counter2, var_counter3, var_counter4, var_counter5, ae_flag, t, tt, q, v, pm_load1, pm_load2, pm_store1, pm_store2
    global pe_flag, ep_flag, pm_flag, aa_flag, pepe_falg, kpm_var, bypass_add_var, bp_add_var
    ...
    try:
        ...
        if x>4:
            ...
            if aa_flag >= len(Instr_Cache):
                ...
                if w5.opcode == "0000000":
                    ...
                    AA_print10 = "Το πεδίο απ του κΑΑ οδηγείται στην είσοδο δεδομένων της πόρτας εγγραφής των καταχωρητών γενικού σκοπού\n"
                    kAA[0] = 0
                    ...
                    if i-3 in bre_index or i-4 in bre_index or i-2 in bre_index_pred or i-3 in bre_index_pred:
                        ...
                        if i-2 in index_bypass_cycle or i-1 in index_bypass_cycle1 or i-1 in index_bypass_cycle2:
                            if w5.name == "ADD":
                                ...
                                kAA[2] = int(w5.r1[1], 16) - int(w5.r2[1])
                                if kAA[2] < 0:
                                    kAA[2] = hex(0)[2:].upper().zfill(2)
                                else:
                                    kAA[2] = hex(kAA[2])[2:].upper().zfill(2)
                                w5.r3[1] = kAA[2]
                            elif w5.name == "AND":
                            ...
                            if w5.name == "ADD":
                                ...
                                kAA[2] = int(temp_reg1_add[v], 16)-int(temp_reg2_add[v])
                    ...
                    AA_print12 = "⬇️Το περιεχόμενο του πεδίου απ γράφεται στον καταχωρητή που δηλώνεται στο πεδίο κε, δηλαδή στον καταχωρητή " + namestr(w5.r3, globals()) + "\n"
                    f = open("Log.txt", 'a', encoding="utf-8")
                    f.write(AA_print9)
                    f.write(AA_print10)
                    f.write(AA_print11)
            ...
            if pm_flag >= len(Instr_Cache):
                ...
                if w4.opcode == "0000000":
                    ...
                    if w4.name == "ADD":
                        ...
                        print(temp_reg2_add)
                        try:
                            kPM[1] = int(temp_reg1_add[t]) - int(temp_reg2_add[t])
                        except IndexError:
                            kPM[1] = int(temp_reg1_add[-1]) - int(temp_reg2_add[-1])
                        
                        if kPM[1] < 0:
                            kPM[1] = 0
                            kPM[1] = hex(0)[2:].upper().zfill(2)  # Result: '00'
                        else:
                            kPM[1] = hex(kPM[1])[2:].upper().zfill(2)
                    elif w4.name == "AND":
                        print("tasos")
                        print(temp_reg1_add)
                        print(temp_reg2_add)
                    ...
                    f.close()
                    try:
                        kPM[0] = "(" + namestr(w4.r1, globals()) + ")" + "." + hex(int(temp_reg1_bre[tt], 16))[2:].upper().zfill(2)
                    except IndexError:
                        kPM[0] = "(" + namestr(w4.r1, globals()) + ")" + "." + hex(int(temp_reg1_bre[-1], 16))[2:].upper().zfill(2)
                    try:
                        kPM[1] = int(temp_reg1_bre[t], 16) - int(temp_reg2_bre[t])
                    except IndexError:
                        kPM[1] = int(temp_reg1_bre[-1], 16) - int(temp_reg2_bre[-1])
                    try:
                        kPM[2] = "(" + namestr(w4.r2, globals()) + ")" + "." + hex(int(temp_reg2_bre[tt], 16))[2:].upper().zfill(2)
                    except IndexError:
                        kPM[2] = "(" + namestr(w4.r2, globals()) + ")" + "." + hex(int(temp_reg2_bre[-1], 16))[2:].upper().zfill(2)
        ...
        if x>2:
            ...
            if ep_flag >= len(Instr_Cache):
                ...
                if w3.opcode == "0000000":
                    ...
                    try:
                        kEP[0] = "(" + namestr(w3.r1, globals()) + ")" + "." + hex(int(temp_reg1_load[t], 16))[2:].upper().zfill(2)
                    except IndexError:
                        kEP[0] = "(" + namestr(w3.r1, globals()) + ")" + "." + hex(int(temp_reg1_load[-1], 16))[2:].upper().zfill(2)
                    try:
                        kEP[1] = "(" + namestr(w3.r2, globals()) + ")" + "." + hex(int(temp_reg2_load[t], 16))[2:].upper().zfill(2)
                    except IndexError:
                        kEP[1] = "(" + namestr(w3.r2, globals()) + ")" + "." + hex(int(temp_reg2_load[-1], 16))[2:].upper().zfill(2)
                    ...
                    try:
                        kEP[0] = "(" + namestr(w3.r1, globals()) + ")" + "." + hex(int(temp_reg1_store[t], 16))[2:].upper().zfill(2)
                    except IndexError:
                        kEP[0] = "(" + namestr(w3.r1, globals()) + ")" + "." + hex(int(temp_reg1_store[-1], 16))[2:].upper().zfill(2)
                    try:
                        kEP[1] = "(" + namestr(w3.r2, globals()) + ")" + "." + hex(int(temp_reg2_store[t], 16))[2:].upper().zfill(2)
                    except IndexError:
                        kEP[1] = "(" + namestr(w3.r2, globals()) + ")" + "." + hex(int(temp_reg2_store[-1], 16))[2:].upper().zfill(2)
                    kEP[2] = 0
                    ...
                    try:
                        kEP[0] = "(" + namestr(w3.r1, globals()) + ")" + "." + hex(int(temp_reg1_add[t], 16))[2:].upper().zfill(2)
                    except IndexError:
                        kEP[0] = "(" + namestr(w3.r1, globals()) + ")" + "." + hex(int(temp_reg1_add[-1], 16))[2:].upper().zfill(2)
                    try:
                        kEP[1] = "(" + namestr(w3.r2, globals()) + ")" + "." + hex(int(temp_reg2_add[t], 16))[2:].upper().zfill(2)
                    except IndexError:
                        kEP[1] = "(" + namestr(w3.r2, globals()) + ")" + "." + hex(int(temp_reg2_add[-1], 16))[2:].upper().zfill(2)
                    ...
                    try:
                        kEP[0] = "(" + namestr(w3.r1, globals()) + ")" + "." + hex(int(temp_reg1_bre[t], 16))[2:].upper().zfill(2)
                    except IndexError:
                        kEP[0] = "(" + namestr(w3.r1, globals()) + ")" + "." + hex(int(temp_reg1_bre[-1], 16))[2:].upper().zfill(2)
                    try:
                        kEP[1] = "(" + namestr(w3.r2, globals()) + ")" + "." + hex(int(temp_reg2_bre[t], 16))[2:].upper().zfill(2)
                    except IndexError:
                        kEP[1] = "(" + namestr(w3.r2, globals()) + ")" + "." + hex(int(temp_reg2_bre[-1], 16))[2:].upper().zfill(2)
                    kEP[2] = 0
                    f = open("Log.txt", 'a', encoding="utf-8")
                    f.write(EP_print5)
                    f.write(EP_print6)
        ...
        if x>1:
            ...
            elif i-1 in bre_index or i in bre_index:
                var_counter1 = 0
                br = pb = 0
                AE_print1 = "-----------------------Βαθμίδα ΑΕ\n"
                AE_print2 = "Καμία εντολή δε χρησιμοποιεί αυτή τη βαθμίδα.\n"
                ...
                if w1.name in ["ADD", "SUB", "AND"]:
                    ...
                    if w2.r3 == w1.r1:
                        if temp_reg1_bre:
                            temp_reg1_bre.pop()
                        temp_reg1_bre.append(w2.r3[1])
                        kEP[0] = "(" + namestr(w1.r1, globals()) + ")" + "." + hex(int(temp_reg1_bre[-1], 16))[2:].upper().zfill(2)
                    if w2.r3 == w1.r2:
                        if temp_reg2_bre:
                            temp_reg2_bre.pop()
                        temp_reg2_bre.append(w2.r3[1])
                        kEP[1] = "(" + namestr(w1.r1, globals()) + ")" + "." + hex(int(temp_reg2_bre[-1], 16))[2:].upper().zfill(2)
                        print("tempp")
                        print(temp_reg1_bre)
                        print(temp_reg2_bre)
                    if choose_bre == 1:
            ...
            if w2.name in ["ADD"]: 
                ...
                if w1.name in ["ADD", "SUB", "AND"]:
                    ...
                    if w2.r3 == w1.r1:
                        if temp_reg1_bre:
                            temp_reg1_bre.pop()
                        result = int(w2.r1[1], 16) - int(w2.r2[1], 16)
                        if result < 0:
                            result = hex(0)[2:].upper().zfill(2)
                        else:
                            result = hex(result)[2:].upper().zfill(2)
                        temp_reg1_bre.insert(t, result)
                        kEP[0] = "(" + namestr(w2.r1, globals()) + ")" + "." + temp_reg1_bre[-1][2:].upper().zfill(2)
                    if w2.r3 == w1.r2:
                        if temp_reg2_bre:
                            temp_reg2_bre.pop()
                        result1 = int(w2.r1[1], 16) - int(w2.r2[1], 16)
                        if result1 < 0:
                            result1 = hex(0)[2:].upper().zfill(2)
                        else:
                            result1 = hex(result1)[2:].upper().zfill(2)
                        temp_reg2_bre.insert(t, result1)
                        kEP[1] = "(" + namestr(w1.r2, globals()) + ")" + "." + temp_reg2_bre[-1][2:].upper().zfill(2)
                        print("temp_reg2_bre")
                        print(temp_reg2_bre)
                    if choose_bre == 1:
                        if int(temp_reg1_bre[-1], 16) - int(temp_reg2_bre[-1], 16) == 0:
