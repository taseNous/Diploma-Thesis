...
def main():
    ...
    label1 = tk.Label(root, text="Welcome! This is a CPU Pipelining Simulator.",
                    font=("Arial", int(18 * font_scale)), fg="#FFFFFF", bg='#2E2E2E')
    label2 = tk.Label(root, text="It shows the data path of a CPU and how pipelining works.",
                    font=("Arial", int(16 * font_scale)), fg="#FFFFFF", bg='#2E2E2E')
    label4 = tk.Label(root, text="Diploma Thesis: Tasos Venos", font=("Arial", int(14 * font_scale)), fg="#FFFFFF", bg='#2E2E2E')
    label5 = tk.Label(root, text="Title: CPU Pipelining Simulator",
                    font=("Arial", int(14 * font_scale)), fg="#FFFFFF", bg='#2E2E2E')
    label6 = tk.Label(root, text="Supervisor Professor: Nikolos Dimitris", font=("Arial", int(14 * font_scale)), fg="#FFFFFF", bg='#2E2E2E')
    ...
    button_commands = [
        ("Instruction Management", lambda: smooth_transition(lambda r: manage_instr.create_button1_window(r, screen_width, screen_height))),
        ("Register Management", lambda: smooth_transition(registers.create_button2_window)),
        ("Memory Management", lambda: smooth_transition(memory.create_button3_window)),
        ("Choose Method", lambda: smooth_transition(lambda r: method.create_button4_window(r, screen_width, screen_height)))
    ]
    ...
    if start == 0:
        button5 = tk.Button(canvas, text="Program Execution", command=show_pipeline, **execute_button_style)
        ...
        button6 = tk.Button(canvas, text="Continue Execution", command=show_pipeline, **execute_button_style)
        button7 = tk.Button(canvas, text="New Execution", command=show_pipeline1, **execute_button_style)
        button_exit = tk.Button(canvas, text="Exit", command=exit, 
                                font=("Arial", int(16 * font_scale)), bg="#ff6666", fg="black", activebackground="#cc0000", activeforeground="white", width=int(screen_width/60))
