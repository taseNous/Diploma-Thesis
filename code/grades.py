import gui_vars, dependencies, table_instructions, manage_instr, cycles

from vars import index1, index2, index3, index4, index5, index6, index8, index7, bp_index, freeze_array
...
def vathmides_nop(canvas, screen_width, screen_height, z, i):
    ...
    global pe_skip, ae_skip, ep_skip, pm_skip, aa_skip, d, x_flag, instr_move, print_again, change_pe_flag1, pe_instr

    Instr_Cache = gui_vars.Instr_Cache
    bre_index = cycles.bre_index
    bre_array_not = cycles.bre_array_not
    change_pe_flag = table_instructions.change_pe_flag
    ...
    def is_aa_in_first_row(tolerance=5):
        """
    ...
    def place_pe_below_pm(canvas, screen_height):
        # Define row height increment
        row_increment = screen_height / 36

        # Get all items in the canvas
        items = canvas.find_all()
        
        # Dictionary to store texts by row
        row_texts = {}

        # Organize texts by rows based on their Y position
        for item in items:
            if canvas.type(item) == "text":
                text = canvas.itemcget(item, "text")
                coords = canvas.coords(item)
                ...
                # Add item to the appropriate row in row_texts
                if row_y not in row_texts:
                    row_texts[row_y] = []
                row_texts[row_y].append((text, item, coords[0], coords[1]))  # (text, item, x, y)

        # Sort row positions to ensure row-by-row processing
        sorted_row_y_positions = sorted(row_texts.keys())

        # Iterate through rows to find the specific conditions
        for i in range(len(sorted_row_y_positions) - 1):
            current_row_y = sorted_row_y_positions[i]
            next_row_y = current_row_y + row_increment  # Y position of row i+1

            # Flags and variables for texts in current and next rows
            has_bre = False
            pm_position = None
            pe_item = None

            # Check for "BRE" and "ΠΜ" in the current row
            for text, item, x, y in row_texts[current_row_y]:
                if text.startswith("BRE"):
                    has_bre = True
                elif text == "ΠΜ":
                    pm_position = (x, y)
            ...
            # Check for "ΠΕ" in the next row
            for text, item, x, y in row_texts.get(next_row_y, []):
                if text == "ΠΕ":
                    pe_item = (item, x, y)

            # If the conditions are met, adjust the position of "ΠΕ"
            if has_bre and pm_position and pe_item:
                pm_x, pm_y = pm_position
                pe_item_id, pe_x, pe_y = pe_item

                # Check if "ΠΕ" is not directly below "ΠΜ"
                if abs(pe_x - pm_x) > 1 or abs(pe_y - (pm_y + row_increment)) > 1:
                    # Move "ΠΕ" directly below "ΠΜ"
                    new_pe_y = pm_y + row_increment
                    canvas.coords(pe_item_id, pm_x, new_pe_y)


    
    # Delete texts that contain ΠΕ, ΑΕ, ΕΠ, ΠΜ, ΑΑ, X in the first row
    def delete_specific_texts():
        items = canvas.find_all()
    ...
    # Delete texts that contain ΠΕ, ΑΕ, ΕΠ, ΠΜ, ΑΑ, X in the last row
    def delete_specific_texts1(i):
        items = canvas.find_all()
        texts_to_delete = ["ΠΕ", "ΑΕ", "ΕΠ", "ΠΜ", "ΑΑ", "Χ"]
        ...
        for item in items:
            if canvas.type(item) == "text":
                ...
                coords = canvas.coords(item)
                # Delete the text if it matches and has the first row's Y position
                if text in texts_to_delete and i == len(Instr_Cache) and coords[1] == screen_height / 7.4482 + 4*(screen_height / 36):
                    canvas.delete(item)
                if text in texts_to_delete and i == len(Instr_Cache) + 1 and coords[1] == screen_height / 7.4482 + 3*(screen_height / 36):
                    canvas.delete(item)
                if text in texts_to_delete and i == len(Instr_Cache) + 2 and coords[1] == screen_height / 7.4482 + 2*(screen_height / 36):
                    canvas.delete(item)
                if text in texts_to_delete and i == len(Instr_Cache) + 3 and coords[1] == screen_height / 7.4482 + 1*(screen_height / 36):
                    canvas.delete(item)
                if text in texts_to_delete and i == len(Instr_Cache) + 4 and coords[1] == screen_height / 7.4482 + 0*(screen_height / 36):
                    canvas.delete(item)
    ...
    def move_isolated_pe_texts(canvas, screen_width, screen_height):
        # Define the distances for isolation checks
        x_distance_threshold = screen_width / 38.4
        y_distance_threshold = screen_height / 36
        move_up_distance = -2 * y_distance_threshold  # Distance to move isolated "ΠΕ" up

        # Get all items in the canvas
        items = canvas.find_all()
        # List to store "ΠΕ" items and their positions
        pe_texts = []

        # Collect all "ΠΕ" texts with their coordinates
        for item in items:
            if canvas.type(item) == "text":
                text = canvas.itemcget(item, "text")
                coords = canvas.coords(item)
                if text == "ΠΕ":
                    pe_texts.append((item, coords[0], coords[1]))  # Store item, x, and y

        # Check each "ΠΕ" text to see if it is isolated
        for pe_item, pe_x, pe_y in pe_texts:
            isolated = True  # Assume "ΠΕ" is isolated until proven otherwise

            # Check for other texts within the specified x and y distance
            for item in items:
                if canvas.type(item) == "text" and item != pe_item:
                    text = canvas.itemcget(item, "text")
                    coords = canvas.coords(item)
                    other_x, other_y = coords[0], coords[1]

                    # Check if the other text is within the x and y distance thresholds
                    if abs(other_x - pe_x) <= x_distance_threshold and abs(other_y - pe_y) <= y_distance_threshold:
                        isolated = False  # "ΠΕ" is not isolated
                        break

            # If "ΠΕ" is isolated, move it up by the specified distance
            if isolated:
                new_y = pe_y + move_up_distance
                canvas.coords(pe_item, pe_x, new_y)
    ...
    if is_aa_in_first_row():
        # if len(Instr_Cache) < 6:
        delete_specific_texts()
        move_texts_up()
    ...
    if i >= 0:
        if i in bre_array_not:
            ...
            if pe_flag > len(Instr_Cache):
                ...
                # pe_flag += 1
                # pe_instr = 1
                # if i-1 in bre_index or i in bre_index_pred:
                #     pe_flag += d_value[index]-1
    if i >= 1:
        if i in bre_array_not or i-1 in bre_array_not:
            ...
            if ae_flag > len(Instr_Cache):
                ...
                position_y2 += screen_height / 36
                # ae_flag += 1
                # if i-2 in bre_index or i -1 in bre_index_pred:
                #     ae_flag += d_value[index]-1
    if i >= 2:
        if i - 2 in bre_array_not or i-1 in bre_array_not:
            ...
            if ep_flag > len(Instr_Cache):
                ...
                position_y3 += screen_height / 36
                # ep_flag += 1
                # if i-3 in bre_index or i-2 in bre_index_pred:
                #     ep_flag += d_value[index]-1
    if i >= 3:
        if i - 2 in bre_array_not or i - 3 in bre_array_not:
            ...
            if pm_flag > len(Instr_Cache):
                ...
                position_y4 += screen_height / 36
                # pm_flag += 1
                # if i-4 in bre_index or i-3 in bre_index_pred:
                #     pm_flag += d_value[index]-1
    if i >= 4:
        if i - 4 in bre_array_not or i - 3 in bre_array_not:
            ...
            if aa_flag > len(Instr_Cache):
                ...
                position_y5 += screen_height / 36
                # aa_flag += 1
                # if i-5 in bre_index or i-4 in bre_index_pred:
                #     aa_flag += d_value[index]-1
...
def vathmides_freeze(canvas, screen_width, screen_height, z, i):
    ...
    global position_x1, position_x2, position_x3, position_x4, position_x5, position_y1, position_y2, position_y3, position_y4, position_y5, index1, index2, index3, index4, index5
    global bp_index, pe1_flag, pe_skip, aee_flag, ae_skip, ep_skip, pm_skip, aa_skip, next_instr, next_l, instr_move, pe_instr, index
...
def vathmides_bypassing_nop(canvas, screen_width, screen_height, z, i):
    ...
    global position_x1, position_x2, position_x3, position_x4, position_x5, position_y1, position_y2, position_y3, position_y4, position_y5, freeze_array, arrow, arrow1, arrow2, arrow3
    global pe_skip, ae_skip, ep_skip, pm_skip, aa_skip, d, x_flag, instr_move, print_again, pe_instr, aee_flag, index3, index5, index6
...
def vathmides_bypassing_freeze(canvas, screen_width, screen_height, z, i):
    ...
    global bp_index, pe_skip, aee_flag, ae_skip, ep_skip, pm_skip, aa_skip, next_instr, next_l, instr_move, pe_instr, index, arrow, arrow2, arrow3, arrow4
    ...
    if i-2 in index4:
        ...
        if any(i-2 - val1 == 3 for val1 in bre_index) or any(abs(val1 - val2) == 4 for val1 in bre_array_not for val2 in index4) or any(abs(val1 - val2) == 5 for val1 in bre_array_not for val2 in index4):
            x1_line = screen_width/5.5652 + 2*(screen_width/38.4)
            y1_line = screen_height/7.4482 
            x2_line = screen_width/5.1063 + 2*(screen_width/38.4)
            y2_line = screen_height/6.1714
    ...
    if i >= 1:
        ...
        if i in bre_array_not or i-1 in bre_array_not:
            ...
            if ae_flag > len(Instr_Cache):
                ...
                # ae_flag += 1
                # if i-3 in bre_index:
                #     ae_flag += d_value[index]-2
                # elif i-1 in bre_index_pred:
                #     ae_flag += d_value[index]-1
    ...
    if i >= 2:
        ...
        if i - 2 in bre_array_not or i-1 in bre_array_not:
            ...
            if ep_flag > len(Instr_Cache):
                ...
                # ep_flag = ep_flag + 1
                # if i-4 in bre_index:
                #     ep_flag += d_value[index]-2
                # elif i-2 in bre_index_pred:
                #     ep_flag += d_value[index]-1
    ...
    if i >= 3:
        ...
        if i - 2 in bre_array_not or i - 3 in bre_array_not:
            ...
            if pm_flag > len(Instr_Cache):
                ...
                # pm_flag = pm_flag + 1
                # if i-5 in bre_index:
                #     pm_flag += d_value[index]-2
                # elif i-3 in bre_index_pred:
                #     pm_flag += d_value[index]-1
    if i >= 4:
        # if i-5 in bre_index:
        #     aa_flag += 1
        if i - 4 in bre_array_not or i - 3 in bre_array_not:
            ...
            if aa_flag > len(Instr_Cache):
                ...
                # if i-3 in bre_index_pred or i-2 in bre_index_pred:
                #     aa_skip = 1
                canvas.create_text(position_x5, position_y5, text="ΑΑ", font=("Arial", int(screen_width/192)))
                position_x5 = position_x5 + screen_width/38.4
                position_y5 = position_y5 + screen_height/36
                # aa_flag = aa_flag + 1
                # if i-6 in bre_index:
                #     aa_flag += d_value[index]-2
                #     index += 1
                # elif i-4 in bre_index_pred:
                #     aa_flag += d_value[index]-1
                #     index += 1
